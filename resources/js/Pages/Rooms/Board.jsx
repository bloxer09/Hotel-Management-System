...
export default function Board({ rooms, roomTypes, housekeepers = [] }) {
    ...
    return (
        <AuthenticatedLayout>
            ...
            {/* ── ROOM DETAIL MODAL ── */}
            <Modal show={!!selectedRoom && !activeModal} onClose={() => setSelectedRoom(null)} maxWidth="lg">
                {selectedRoom && (
                    <>
            ...
            {/* ── ADD ROOM MODAL ── */}
            <Modal show={activeModal === 'add' && isAdmin} onClose={() => { setActiveModal(null); setSelectedRoom(null); }} maxWidth="md">
                <div className="p-6">
                    ...
                    <form onSubmit={handleAddSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea value={addForm.data.notes}
                                ...
                                rows="3" placeholder="e.g. Near elevators, ocean view..."
                                className="w-full bg-[#0f172a] border border-[#334155] rounded-xl text-slate-100 px-3 py-2 text-xs focus:outline-none focus:border-brand-500 resize-none" />
                        </div>

                        {/* Room Photo drag-and-drop zone */}
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-slate-400 mb-1 block">Room Photo (Optional)</label>
                            <div className="relative group border border-dashed border-[#334155] hover:border-brand-500/60 rounded-xl p-4 bg-[#0f172a] hover:bg-[#0f172a]/80 transition-all flex flex-col items-center justify-center text-center cursor-pointer min-h-[90px]">
                                <input
                                    type="file"
                                    onChange={e => addForm.setData('photo', e.target.files[0])}
                                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                />
                                <Paperclip size={18} className="text-slate-400 group-hover:text-brand-400 transition-colors mb-1.5" />
                                <div className="text-xs font-bold text-slate-200 group-hover:text-slate-100">
                                    {addForm.data.photo ? addForm.data.photo.name : 'Choose Photo or Drag & Drop'}
                                </div>
                                <p className="text-[9px] text-slate-500 mt-1">PNG, JPG up to 4MB</p>
                            </div>
                            {addForm.errors.photo && <span className="text-[10px] text-red-400 font-semibold">{addForm.errors.photo}</span>}
                        </div>

                        <div className="flex justify-end gap-3 pt-2">
                            <button type="submit" disabled={addForm.processing}
                                className="px-5 py-2.5 bg-brand-600 hover:bg-brand-500 rounded-xl text-white font-bold text-sm transition-all disabled:opacity-50 shadow-md">
                                Add Room
                            </button>
                        </div>
                    </form>
            ...
            {/* ── EDIT ROOM MODAL ── */}
            <Modal show={activeModal === 'edit' && !!selectedRoom && isAdmin} onClose={() => { setActiveModal(null); setSelectedRoom(null); }} maxWidth="lg">
                {selectedRoom && (
                    <>
                        <div className="flex items-center justify-between border-b border-[#334155] px-6 py-4 shrink-0">
                            ...
                            <form onSubmit={handleEditSubmit} className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <CustomSelect value={editForm.data.status}
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-400 mb-1 block">Room Type *</label>
                                    <CustomSelect value={editForm.data.room_type_id}
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <div className="relative group border border-dashed border-[#334155] hover:border-brand-500/60 rounded-xl p-4 bg-[#0f172a] hover:bg-[#0f172a]/80 transition-all flex flex-col items-center justify-center text-center cursor-pointer min-h-[90px]">
                                        <input
                                            type="file"
                                            onChange={e => {
                                                editForm.setData('photo', e.target.files[0]);
                                                editForm.setData('remove_photo', false);
                                            }}
                                            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                        />
                                        <Paperclip size={18} className="text-slate-400 group-hover:text-brand-400 transition-colors mb-1.5" />
                                        <div className="text-xs font-bold text-slate-200 group-hover:text-slate-100">
                                            {editForm.data.photo ? editForm.data.photo.name : 'Choose Photo or Drag & Drop'}
                                        </div>
                                        <p className="text-[9px] text-slate-500 mt-1">PNG, JPG up to 4MB</p>
                                    </div>
                                    {selectedRoom.photo_url && !editForm.data.remove_photo && (
                                        <div className="flex items-center justify-between mt-2 p-2 rounded-lg bg-[#0f172a]/60 border border-[#334155]/60 text-[10px]">
                                            <span className="text-slate-350 truncate max-w-[200px]">Current Photo</span>
                                            <button
                                                type="button"
                                                onClick={() => {
                                    ...
                                    {editForm.data.remove_photo && (
                                        <span className="text-[10px] text-amber-500 italic mt-1 block font-medium">Existing photo will be deleted on save.</span>
                                    )}
                                    {editForm.errors.photo && <span className="text-[10px] text-red-400 font-semibold">{editForm.errors.photo}</span>}
                                </div>

                                <div className="flex justify-end gap-3 pt-2">
                                    <button type="button" onClick={() => { setActiveModal(null); setSelectedRoom(null); }} className="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-sm font-bold font-outfit">Cancel</button>
                                    <button type="submit" disabled={editForm.processing}
                                        className="px-5 py-2.5 bg-brand-600 hover:bg-brand-500 text-slate-50 rounded-xl text-sm font-bold font-outfit shadow-md disabled:opacity-50">
                                        Save Changes
                                    </button>
                                </div>
                            </form>
